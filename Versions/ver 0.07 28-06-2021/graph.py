...
def graph(symbol, months, y_ax):
    ...
    def plot(plotnum):
        ...
        for i in range(months):
            ...
            if plotnum == 1:
                ...
                y_axis = y_ax
                # print("FETCH",fetch(False,False,symbol,"usd",12), "FETCH")
                y_axis = y_axis[::-1]
                y_init = y_axis[0]
                break
        
            elif plotnum == 4:
                y_axis = y_axis + [y_plot[3][0]]*months
                y_init = y_axis[0]
                break
                ...
                y_axis = y_axis + [initial_investment / investment * float(y_plot[4][i])]
                y_init = y_axis[0]
                ...
                y_axis = y_axis + [float(y_plot[2][i]) * float(y_plot[3][i])]
                y_init = y_axis[0]
            
            elif plotnum == 7:
                y_axis = y_axis + [float(y_plot[2][i]) * float(y_plot[4][i])]
                y_init = y_axis[0]
    ...
    with open(r'Resources/data.json', 'w') as file:
        json_string = json.dumps(dict)
        file.write(json_string)
    with open(r'Resources/data.json') as file:
        data = json.load(file)
    ...
    plt.plot(x_axis,y_plot3, color='k', label='Token Value(' + str(round( float(y_plot3[0]) ,4)) + ' ' + str(round( float(y_plot3[len(y_plot3)-1]) ,4)) + ')')
    ...
    plt.fill_between(x_axis, y_plot5, y_plot4,
                    where=(y_plot5 <= y_plot4), interpolate=True, color='orange', alpha=0.5, label='Lower Profits')
...
if __name__ == "__main__":
    graph(input("Crypto symbol: "), int(input("Whole months: ")), fetch(False,False,"SOL","usd",9))