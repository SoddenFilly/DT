...
def fetch(url_local_input, url_symbol, url_market, month_cap):
    ...
    while url:
        try:
            if url_local_input == True:
                ...
                url = "https://www.alphavantage.co/query?function=DIGITAL_CURRENCY_MONTHLY&symbol=" + input("Symbol: ") + "&market=" + input("Market: ") + "&apikey=" + api_key
        ...
        except: print("You may have misspelt somthing or that currency is not available,\nPlease try again.")
        else: break
    ...
    while run:
        ...
        try:
            value = dict_json["Time Series (Digital Currency Monthly)"][str(year)+"-"+zero+str(month)+"-"+str(day)]["4a. close (NZD)"]
